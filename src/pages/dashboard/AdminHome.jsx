...
import { HiOutlineUsers, HiOutlineUserGroup, HiOutlineCurrencyDollar, HiOutlineClipboardDocumentList, HiOutlineClipboardDocumentCheck, HiOutlineBanknotes, HiOutlineChartBar } from 'react-icons/hi2';
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {[
          ...
          { label: 'Total Tasks', value: stats.totalTasks || 0, icon: HiOutlineClipboardDocumentList, color: 'text-[#0EA5E9]', bg: 'bg-[#0EA5E9]/5' },
          { label: 'Submissions', value: stats.totalSubmissions || 0, icon: HiOutlineClipboardDocumentCheck, color: 'text-[#8B5CF6]', bg: 'bg-[#8B5CF6]/5' },
          { label: 'Total Revenue', value: `$${stats.totalRevenue?.toFixed(2) || '0.00'}`, icon: HiOutlineBanknotes, color: 'text-[#10B981]', bg: 'bg-[#10B981]/5' }
        ].map((item, idx) => (
          <motion.div
            key={idx}
            initial={{ opacity: 0, y: 20 }}
